...
class DescriptorRules {
  ...
  MessageRules? getMessageRules(String messageTypeName) {
    ...
    for (final file in descriptorSet.file) {
      ...
      if (message != null && message.hasOptions()) {
        ...
        try {
          final rules =
              message.options.getExtension(Validate.message) as MessageRules?;
          if (rules != null) {
  ...
  DescriptorProto? _findMessageDescriptor(
      FileDescriptorProto file, String fullName) {
  ...
  void _cacheMessageFieldRules(
      String messageTypeName, DescriptorProto message) {
    ...
    for (final field in message.field) {
      if (field.hasOptions()) {
        try {
          ...
          var rules =
              field.options.getExtension(Validate.field_1159) as FieldRules?;
          if (rules != null) {
        ...
        for (final oneof in message.oneofDecl) {
          if (oneof.hasOptions()) {
            try {
              ...
              final rules =
                  oneof.options.getExtension(Validate.oneof) as OneofRules?;
              if (rules != null) {
  ...
  EnumDescriptorProto? _findNestedEnum(
    ...
    final messagePrefix =
        package.isNotEmpty ? '${package}.${message.name}' : message.name;
    ...
    for (final nestedMessage in message.nestedType) {
      final enumDesc =
          _findNestedEnum(nestedMessage, enumTypeName, messagePrefix);
      if (enumDesc != null) {
        return enumDesc;
      }
    }
